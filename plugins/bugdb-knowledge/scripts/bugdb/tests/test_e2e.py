...
def test_e2e_full_lifecycle(tmp_path):
    ...
    # 4. feedback 成功
    fb = _run(["feedback", "--id", str(old_id), "--result", "success"], db)
    assert fb.returncode == 0
    g = json.loads(_run(["get", "--id", str(old_id)], db).stdout)
    ...
    er = _run(["export", "--output", str(out)], db)
    assert er.returncode == 0
    db2 = tmp_path / "e2e2.db"
    ir = _run(["import", "--input", str(out)], db2)
    assert ir.returncode == 0
    lst = _run(["list", "--status", "all"], db2)
