...
def test_cli_update(tmp_path):
    ...
    u = _run(["update", "--id", str(bug_id), "--solution", "new sol", "--confidence", "70"], tmp_path)
    assert u.returncode == 0
    g = _run(["get", "--id", str(bug_id)], tmp_path)
...
def test_cli_delete_soft_and_restore(tmp_path):
    ...
    bug_id = json.loads(add.stdout)["id"]
    d = _run(["delete", "--id", str(bug_id)], tmp_path)
    assert d.returncode == 0, d.stderr
    g = _run(["get", "--id", str(bug_id)], tmp_path)
    assert json.loads(g.stdout)["status"] == "archived"
    rr = _run(["restore", "--id", str(bug_id)], tmp_path)
...
def test_cli_feedback(tmp_path):
    ...
    bug_id = json.loads(add.stdout)["id"]
    fb = _run(["feedback", "--id", str(bug_id), "--result", "success"], tmp_path)
...
def test_cli_deprecate(tmp_path):
    old_id = _add_bug(tmp_path)
    new_id = _add_bug(tmp_path)
...
def test_cli_obsolete(tmp_path):
    bug_id = _add_bug(tmp_path)
    r = _run(["obsolete", "--id", str(bug_id), "--reason", "API gone"], tmp_path)
    assert r.returncode == 0
    g = _run(["get", "--id", str(bug_id)], tmp_path)
