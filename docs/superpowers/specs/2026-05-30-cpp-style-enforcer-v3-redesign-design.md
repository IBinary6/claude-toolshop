...
| 默认模式 | `incremental`（只对新文件走三件套）；`full` 可选 |
...
| 场景 | 配置 | 行为 |
|---|---|---|
| **新项目** | `mode: full` | **所有文件强制全套**：clang-format（**含 #include 排序，必须规范**）+ 版权 + cpplint + BOM |
| **老项目·新文件** | `mode: incremental` 且 `!isTracked` | **同样强制全套**：格式化+版权+cpplint+BOM（新写的代码就该规范，也修复问题） |
| **老项目·老文件** | `mode: incremental` 且 `isTracked` | **只看 BOM**：缺 BOM 才补，已有 BOM 不动；**格式化/版权/cpplint 一概不碰** |

- 非 git 仓库：所有文件视为「新」（走配置规定的检查）。
- `applyTriple = (mode==='full') || (mode==='incremental' && isNew(file))`。
- **关键决策（用户明确）**：
  - 走全套的文件（新项目所有文件 + 老项目新文件）→ clang-format **正常按 Google 风格格式化、正常排序 #include**，必须规范。
  - 老项目老文件 → **根本不进 clang-format 步骤**，因此 include 永不被排序。这是本插件的**核心目的**：VS 默认会对所有文件排序 include，殃及老项目老文件；本插件让老文件彻底不进格式化，只补 BOM。
  - 因此**不需要 `--sort-includes=0`、不需要 sortIncludes 开关**——区分在"文件进不进 clang-format"，而非排序参数。
...
- **git add 会触发档位翻转**：incremental 模式下，新文件首次编辑（未跟踪）走全套、被格式化；用户 `git add` 后该文件变「已跟踪」，再次编辑即降级为「只补 BOM、不再格式化」。这是 `!isTracked` 判定的自然结果，符合「存量代码不动」的意图。
...
|---|---|---|
| 1. clang-format | `steps/clang_format.js` | `applyTriple && checks.clangFormat` 且 clang-format 在 PATH |
...
| 4. cpplint | `steps/cpplint.js` | `applyTriple && checks.cpplint` 且 python 在 PATH |
...
- **clang-format**：走全套的文件按 Google 风格格式化（含 #include 排序）。老项目老文件不进此步。
  - **调用方式（内置 Google 兜底，绝不碰项目文件）**：`-style=file -fallback-style=Google`。项目有 `.clang-format` → 读它（VS/clangd 同源一致）；没有 → 用 clang-format **内置** Google 风格兜底，**不写任何 `.clang-format` 到项目**。
  - 不传 `--sort-includes`（命令行开关跨版本不可靠，无必要）。
  - **BOM 感知（CRITICAL，与 copyright 同构）**：文件此时**可能已带 BOM**（已跟踪的新文件再次编辑、或 CMake 项目里曾被其它工具加过）。流程：`读文件 → bom_util.stripBom → 对无 BOM 正文跑 clang-format(stdin/stdout) → 与无 BOM 正文 diff → 仅当有变化时按 bom_util.restoreBom(hadBom, formatted) 写回`。**绝不把带 BOM 字节直接喂给 clang-format**（避免某些版本把 BOM 当内容或剥掉导致 BOM 丢失/重复）。CMake 项目里走 clang-format 的文件，BOM 由此步在 strip/restore 间**原样保全**，即使后续 BOM 步骤被 CMake 门控跳过也不丢 BOM。
...
违规处理：
- **所有违规一视同仁**（不按 confidence 分级过滤；cpplint 默认已按其规则集输出）→ 有违规即 `blockClaude(reason)`：exit 0 + stdout `{"decision":"block","reason":<去重后前5条+修复指令>}`。
- `company` 空 或 `copyright` 关 → 设置过滤屏蔽 `legal/copyright`，避免缺头误拦。
...
- 仅 `ensureUserTemplate(PLUGIN_DEFAULT_TEMPLATE)`：用户模板不存在则从出厂默认复制。
...
- 旧文件（已跟踪）incremental → 只补 BOM、不格式化/不 lint。
